...
const fetchEvent = async () => {
  try {
    const response = await axios.get(`https://fishmuggers-is216-express.onrender.com/api/events/${eventId}`);
...
const fetchInterestedUsers = async () => {
  try {
    ...
    const responses = await Promise.all(
      userIds.map(uid => axios.get(`https://fishmuggers-is216-express.onrender.com/api/users/${uid}`))
    );
...
const fetchFriends = async () => {
  try {
    ...
    const response = await axios.get(`https://fishmuggers-is216-express.onrender.com/api/events/${userUid}/friends`);
